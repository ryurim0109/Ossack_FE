...
export const getSOListDB = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const responce: DataType = await instance.get(
        `/estates/${searchInfo.pageno}?query=${searchInfo.keyword}&depositlimit=${searchInfo.depositlimit}&feelimit=${searchInfo.feelimit}&monthly=${searchInfo.monthly}`
      );
      thunkAPI.dispatch(
        getSOList({
...
export const getShareListDB = createAsyncThunk(
  ...
  async (shareInfo: { keyword: string; pageno: number }, thunkAPI) => {
    try {
      thunkAPI.dispatch(is_loaded(false));
      const responce: ShareType = await instance.get(
        `/sharedoffices?query=${shareInfo.keyword}&pagenum=${shareInfo.pageno}`
      );
      thunkAPI.dispatch(
        getShareList({
          totalpage: responce.data.totalpage,
...
export const officeSlice = createSlice({
  ...
  reducers: {
    ...
    shareClickLike: (state, action: PayloadAction<any>) => {
      let numArr: Array<any> = [];
      state.share_list.sharedOfficeResponseDtos.filter((val, idx) => {
        if (val.shareofficeid === action.payload.shareofficeid) {
          return numArr.push(idx);
        }
        return false;
      });
      state.share_list.sharedOfficeResponseDtos[numArr[0]].mylike = true;
      return;
    },
    shareDeleteLike: (state, action: PayloadAction<any>) => {
      ...
      state.share_list.sharedOfficeResponseDtos[numArr[0]].mylike = false;
      return;
    },
